...
export function RoadmapArrange({
  ...
  const actions: {
    ...
    },
    {
      ...
      note: 'Move bars that finished before now up to now — they had slipped and were still drawn as if they had not.',
      views: ['timeline'],
      run: () => {
        const p: Proposal = proposeCatchUp(items);
    ...
    },
    {
      ...
      note: 'Hand one unclaimed bar from the busiest area to the emptiest. Claimed work never moves.',
      views: ['timeline'],
      run: () => {
    ...
    },
    {
      ...
      note: 'Reorder each lane so it runs S, A, B, C — the same sort the run queue uses.',
      views: ['timeline'],
      run: () => {
        const p: Proposal = proposeByTier(items);
    ...
    },
    {
      ...
      note: canRead
        ? 'Reads what these items actually are and says what must come before what. The only action here that asks Claude — on your own host, through the terminal daemon.'
    ...
    <div className={`ra${open ? ' open' : ''}`}>
      <button className="ra-head" onClick={onToggle} aria-expanded={open}>
        ...
        <span className="ra-hint">
          {open
            ? 'Arithmetic over what is on the board, plus one ✦ read of it. Nothing is saved until you apply it.'
