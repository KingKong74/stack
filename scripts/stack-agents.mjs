...
// agents`). The human-facing surface for server/src/agents.js's pure engine:
...
async function cmdList(argv) {
  ...
  let res;
  try {
    res = await request(cfg, 'GET', '/api/agents');
  } catch {
...
async function cmdSet(argv) {
  ...
  let listRes;
  try {
    listRes = await request(cfg, 'GET', '/api/agents');
  } catch {
    return fail(`could not reach ${cfg.api} — is the Stack API up?`);
  }
  ...
  try {
    if (exists) {
      res = await request(cfg, 'PATCH', `/api/agents/${encodeURIComponent(key)}`, patch);
    } else {
      ...
      res = await request(cfg, 'POST', '/api/agents', { key, ...patch });
    }
  } catch {
    return fail(`could not reach ${cfg.api} — is the Stack API up?`);
  }
...
async function cmdReset(argv) {
  ...
  let res;
  try {
    res = await request(cfg, 'DELETE', `/api/agents/${encodeURIComponent(key)}`);
  } catch {
    return fail(`could not reach ${cfg.api} — is the Stack API up?`);
  }
