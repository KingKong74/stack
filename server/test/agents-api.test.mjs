...
import { resolveSpawn } from '../src/agents.js';
...
(async () => {
  ...
  // ---- Catalogue -----------------------------------------------------
  const fresh = await callOk('/api/agents');
  const byKey = (list, key) => list.find((p) => p.key === key);

  checkTrue('GET /api/agents returns both builtins', !!byKey(fresh.profiles, 'executor') && !!byKey(fresh.profiles, 'reviewer'));
  ...
  // ---- Agent creation --------------------------------------------------
  const created = await callOk('/api/agents', {
    method: 'POST',
  ...
  {
    const { status } = await call('/api/agents', {
      method: 'POST',
  ...
  const afterCreate = await callOk('/api/agents');
  checkTrue('the new profile appears in GET /api/agents', !!byKey(afterCreate.profiles, 'docs-writer'));
  {
  ...
  {
    const { status, body } = await call('/api/agents', { method: 'POST', body: { key: 'no-prompt-profile' } });
    check('POST with no prompt → 400', status, 400);
    checkTrue('the 400 error mentions the prompt', /prompt/i.test(body?.error || ''));
  }
  {
    const { status, body } = await call('/api/agents', {
  ...
  {
    const { status } = await call('/api/agents', { method: 'POST', body: { key: 'Bad_Key', prompt: 'x' } });
    check('POST with a bad key → 400', status, 400);
  }

  // ---- Parameter configuration ------------------------------------------
  {
    const { status, body } = await call('/api/agents/docs-writer', { method: 'PATCH', body: { model: 'claude-haiku' } });
  ...
  {
    const { status, body } = await call('/api/agents/executor', { method: 'PATCH', body: { model: 'claude-opus-custom' } });
  ...
  {
    const after = await callOk('/api/agents');
  ...
  {
    const { status, body } = await call('/api/agents/executor', { method: 'DELETE' });
  ...
  {
    const { status } = await call('/api/agents/docs-writer', { method: 'DELETE' });
  ...
  {
    const { status } = await call('/api/agents/no-such-profile-key', { method: 'PATCH', body: { model: 'x' } });
...

  {
    ...
    const catalogue = await callOk('/api/agents');
