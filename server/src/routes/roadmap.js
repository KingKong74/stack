...
// checks. It stores onto the run it describes (composeReviewBrief +
// storeReviewBrief, shared with the automatic write). This route is the
// explicit RE-ASK — a human pressing ✧ Brief to refresh what the Review room
// shows inline; the automatic brief is written at run end, unattended
// (autoReviewBrief in routes/autopilot.js). 503 keyless.
roadmap.post('/:id/review-brief', async (req, res) => {
  ...
  try {
    const { brief, runId } = await composeReviewBrief(req.project, item);
    if (!brief) return res.status(502).json({ error: 'Gemini returned nothing usable.' });
    // The re-ask answers first, and a failed write costs the inline refresh,
    // never the answer — the human already paid for this brief, so a DB
    // hiccup while storing it must not turn a good answer into a 502.
    storeReviewBrief(req.project.id, runId, brief).catch(() => {});
    res.json(brief);
  } catch (err) {
    res.status(err.httpStatus || 502).json({ error: err.message || 'Gemini call failed.' });
  }
