...
// manual Run-now presses) and hands jobs out under three gates (#335, #267): a
// tunable fleet-wide concurrency cap (autopilotWorkers, 0 = unlimited), a fixed
// per-project serialisation and the area lane (never tunable — see the comment
// above CLAIM_NEXT_SQL). All times are the DISPATCHER's local clock, passed in
// as ?local=YYYY-MM-DDTHH:MM&dow=N — the server's own TZ never matters.
// #243 adds an `advise` lane: a read-only job that runs the host-side merge
// advisor against a branch and reports its text back onto the job row.
// ---------------------------------------------------------------------------
